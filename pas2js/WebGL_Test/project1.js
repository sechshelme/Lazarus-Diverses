...
rtl.module("program",["System","browserconsole","BrowserApp","JS","Classes","SysUtils","Web","Mat4","MemoryBuffer","GLUtils","GLTypes","webgl","Math"],function () {
  "use strict";
  var $mod = this;
  rtl.recNewT(this,"GLVertex2",function () {
    this.$new = function () {
  ...
  $mod.$main = function () {
    ...
    $mod.canvas.width = 200;
    $mod.canvas.height = 200;
    ...
    $mod.vertexShaderSource = "" + pas.System.LineEnding + "attribute vec2 inPos; varying vec2 pos;  void main(){ gl_Position = vec4(inPos, 1.0, 1.0);   pos = inPos.xy;}";
    $mod.fragmentShaderSource = "" + pas.System.LineEnding + "precision highp float; varying vec2 pos;  void main(void){  gl_FragColor = vec4(pos, 0.0, 1.0); }";
    ...
    $mod.shader.Link();
    $mod.shader.Use();
    $mod.gl.clearColor(0.3,0.0,0.0,1);
    ...
    $mod.gl.vertexAttribPointer(0,2,5126,false,$mod.stride,$mod.offset);
    $mod.offset += pas.GLUtils.GLSizeof(5126) * 2;
    $mod.gl.enableVertexAttribArray(1);
    $mod.gl.vertexAttribPointer(1,4,5121,true,$mod.stride,$mod.offset);
    $mod.offset += pas.GLUtils.GLSizeof(5121) * 4;
    window.requestAnimationFrame($mod.UpdateCanvas);
  };
});
